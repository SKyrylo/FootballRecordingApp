...
if __name__ == "__main__":
    ...
    with open("custom_data/train.txt", "r") as read_file:
        lines_train = read_file.readlines()

    with open("custom_data/val.txt", "r") as read_file:
    ...
    print(len(lines_train), len(lines_val))
    train_Xs = {os.path.join("custom_data", "train_data", l.split(" ")[0]):int(l.split(" ")[1]) for l in lines_train}
    val_Xs = {os.path.join("custom_data", "val_data", l.split(" ")[0]):int(l.split(" ")[1]) for l in lines_val}
    ...
    train_dataset = KickPunchDataset(list(train_Xs.keys()), list(train_Xs.values()), 15, transform=transform) # old
    test_dataset = KickPunchDataset(list(val_Xs.keys()), list(val_Xs.values()), 15, transform=transform_test) # old
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        if current_val_accuracy > best_accuracy:
            torch.save(model, f"weights/movinet_a2_base_custom_data_ft_{epoch}_epoch_val_acc_{str(round(current_val_accuracy, 4)).replace('.', '_')}.pth")
            if os.path.exists(f"weights/movinet_a2_base_custom_data_ft_{last_epoch_checkpoint}_epoch_val_acc_{str(round(best_accuracy, 4)).replace('.', '_')}.pth"):
                os.remove(f"weights/movinet_a2_base_custom_data_ft_{last_epoch_checkpoint}_epoch_val_acc_{str(round(best_accuracy, 4)).replace('.', '_')}.pth")
