...
def train_iter(model, optimz, data_load, loss_val, epoch, class_weight=None, scheduler=None, loss_fn=None):
    ...
    for i, sample in enumerate(data_load): # pbar:
        ...
        if loss_fn is None:
            out_cls = F.log_softmax(result[0], dim=1)
            loss_cls = F.nll_loss(out_cls, sample["label_cls"].cuda(), weight=class_weight)
        ...
        optimz.zero_grad()
        model.clean_activation_buffers()
        _, pred = torch.max(out_cls, dim=1)
        epoch_loss_cls += loss_cls.item()
        epoch_loss_loc += loss_loc.item()
        accuracy = calc_acc(sample["label_cls"].cuda(), pred.cuda())
        mask = out_loc[:, 0] > out_loc[:, 1]
        out_loc[mask] = 0
        ...
        if i % 50 == 0:
            print(f"{i+1} of {samples//batch_size + 1} batches passed. Cls. Loss: {round(epoch_loss_cls / (i + 1), 5)}, Loc. Loss {round(epoch_loss_loc / (i + 1), 5)}, Cls. Accuracy: {round(epoch_acc / (i + 1), 5)}, tIoU {round(epoch_tiou / (i + 1), 5)}")
...
def evaluate(model, data_load, loss_val, loss_fn=None):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(data_load): # pbar:
            ...
            result = model(res.cuda())
            if loss_fn is None:
                output_cls = F.log_softmax(result[0], dim=1)
                loss_cls = F.nll_loss(output_cls, sample["label_cls"].cuda(), reduction='sum')
            ...
            _, pred = torch.max(output_cls, dim=1)
            
            tloss_cls += loss_cls.item()
            tloss_loc += loss_loc.item()
            csamp += pred.eq(sample["label_cls"].cuda()).sum()
            mask = output_loc[:, 0] > output_loc[:, 1]
            output_loc[mask] = 0
    ...
    aloss_cls = tloss_cls / samples
    aloss_loc = tloss_loc / samples
    loss_val.append(aloss_cls + aloss_loc)
    print('\nAverage test cls. loss: ' + '{:.4f}'.format(aloss_cls) + '  Accuracy:' + '{:5}'.format(csamp) + '/' + '{:5}'.format(samples) + ' (' + '{:4.2f}'.format(100.0 * csamp / samples) + '%)  Average test loc. loss: ' + '{:.4f}'.format(aloss_loc) + '  tIoU: ' + '{:4.2f}'.format(100.0 * tiou_e / samples) + '%\n')
    return csamp.item() / samples, tiou_e / samples
...
if __name__ == "__main__":
    ...
    with open("train_dataset_localization_10K.json", "r") as rfile:
        train_dict = json.load(rfile)

    with open("val_dataset_localization_10K.json", "r") as rfile:
        val_dict = json.load(rfile)

    with open("test_dataset_localization_10K.json", "r") as rfile:
